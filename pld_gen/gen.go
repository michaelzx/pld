...
import (
	...
	"github.com/michaelzx/pld/pld_arrays"
	"github.com/michaelzx/pld/pld_config"
	"github.com/michaelzx/pld/pld_fs"
	"github.com/michaelzx/pld/pld_gen/db_meta"
	"log"
	"os"
	"strings"
)
...
func (g *Generator) Gen() {
	...
	pld_fs.CreateIfNotExist(g.OutDir)
	sqlType := "mysql"
...
func (g *Generator) getFields(tableName string, pks []string, comments map[string]string) []string {
	...
	for _, c := range columns {
		...
		if pld_arrays.IndexOfString(pks, key) >= 0 {
			tagInfoItems = append(tagInfoItems, "pk")
		}
		if pld_arrays.IndexOfString(autoKeys, key) >= 0 {
			tagInfoItems = append(tagInfoItems, "auto")
		}
		if key == "logic_delete" {
			tagInfoItems = append(tagInfoItems, "lgc_del")
		}
...
func (g *Generator) checkTables() {
	...
	for _, table := range g.Tables {
		if pld_arrays.IndexOfString(ts, table) == -1 {
